...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://blogz:League12@localhost:8889/blogz'
app.config['SQLALCHEMY_ECHO'] = True

db = SQLAlchemy(app)

app.secret_key = 'ljkng9324hgbioBV(*IYGWBfbn0e4gb'
...
def index():
    ...
    if users:
        ...
        flash('The blog has no user! Sign up now!')
...
def show_blog():
    ...
    if blog_id:
        ...
        return render_template('blog.html',posts=user_posts(user),user=user)

    else:

        return render_template('blog.html', title="Blogz", posts=all_posts())
...
def post_new():
    ...
    if request.method == 'POST':
        ...
        return redirect('/blog?id=' + new_id + '&user=' + str(owner.user_id))
